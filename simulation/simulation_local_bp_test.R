...
run_task <- function(task_id, input_file, main_wd) {
 ...
 # })
  ...
  r_bp_005 <- run_method("Bayesian PROCOVA (Gibbs)", {
    ...
    data.frame(
      method="gibbs", model="Bayesian_PROCOVA - 0.005",
  ...
  r_bp_001 <- run_method("Bayesian PROCOVA (Gibbs)", {
    bp_fit <- bayesian_procova_analysis(
      df_historical = df_h, df_current = df_c,
      model_fun = prognostic_model_superlearn,
      seed  = seed + 300000L + sim_seed_offset,
    ...
    data.frame(
      method="gibbs", model="Bayesian_PROCOVA - 0.001",
      t(summ_treat_bayesian_procova(bp_fit)),
      row.names=NULL, stringsAsFactors=FALSE
    )
  })

  r_bp_01 <- run_method("Bayesian PROCOVA (Gibbs)", {
    bp_fit <- bayesian_procova_analysis(
      df_historical = df_h, df_current = df_c,
      model_fun = prognostic_model_superlearn,
      seed  = seed + 300000L + sim_seed_offset,
    ...
    data.frame(
      method="gibbs", model="Bayesian_PROCOVA - 0.01",
      t(summ_treat_bayesian_procova(bp_fit)),
      row.names=NULL, stringsAsFactors=FALSE
    )
  })

  r_bp_05 <- run_method("Bayesian PROCOVA (Gibbs)", {
    bp_fit <- bayesian_procova_analysis(
      df_historical = df_h, df_current = df_c,
      model = prognostic_model_superlearn,
      seed  = seed + 300000L + sim_seed_offset,
    ...
    data.frame(
      method="gibbs", model="Bayesian_PROCOVA - 0.05",
      t(summ_treat_bayesian_procova(bp_fit)),
      row.names=NULL, stringsAsFactors=FALSE
    )
  })

r_bp_jags_005 <- run_method("Bayesian PROCOVA (JAGS)", {
  withCallingHandlers(
    tryCatch({
      bp_fit <- bayesian_procova_jags(
        df_historical = df_h, df_current = df_c,
        model_fun = prognostic_model_superlearn,
        seed  = seed + 300000L + sim_seed_offset,
        K0H   = 0.005
      ...
      data.frame(
        method = "jags", model = "Bayesian_PROCOVA - JAGS - 0.005",
        t(summ_treat_bayesian_procova(bp_fit)),
        row.names = NULL, stringsAsFactors = FALSE
      )
    }, error = function(e) {
      log_msg("JAGS ERROR (K0H=0.005): %s", conditionMessage(e))
      NULL
    }),
    warning = function(w) {
      log_msg("JAGS WARNING: %s", conditionMessage(w))
      invokeRestart("muffleWarning")
    }
  )
})

r_bp_jags_01 <- run_method("Bayesian PROCOVA (JAGS)", {
  withCallingHandlers(
    tryCatch({
      bp_fit <- bayesian_procova_jags(
        df_historical = df_h, df_current = df_c,
      ...
      data.frame(
        method = "jags", model = "Bayesian_PROCOVA - JAGS - 0.01",
        t(summ_treat_bayesian_procova(bp_fit)),
        row.names = NULL, stringsAsFactors = FALSE
      )
    }, error = function(e) {
      log_msg("JAGS ERROR (K0H=0.01): %s", conditionMessage(e))
      NULL
    }),
    warning = function(w) {
      log_msg("JAGS WARNING: %s", conditionMessage(w))
  ...
  results_list <- list(r_bp_005, r_bp_01, r_bp_001, r_bp_05, r_bp_jags_005, r_bp_jags_01) #list(r_glm, r_stan, r_stan_oracle, r_comm, r_alasso, r_sam, r_bp, r_bp_conservative)
...
cl         <- makeCluster(n_cores, outfile = "cluster_log.txt")
start_time <- Sys.time()
